...
<ParamField path="shopifyOrderIdField" type="string" required>
The field in Brightpearl sales orders that stores the Shopify order ID. This field is used to locate the original sales order when creating a sales credit.

**Common values:**
- `customerRef` (most common)
- `reference`
- `externalRef`

**Important:** This field should contain the Shopify order ID in the format `SO{order_id}` (e.g., `SO1234567890`).
</ParamField>
</Step>
...
3. Check the return timeline in Redo for a Brightpearl sales credit creation entry
4. Log in to Brightpearl to verify the sales credit was created
5. Confirm the sales credit:
...
<Info>
**Green Returns:** Returns where all products are being exchanged (green returns with no refund) are automatically skipped, as they don't require sales credits in Brightpearl.
</Info>

<Info>
**Duplicate Prevention:** The integration checks if a sales credit already exists for a return before attempting to create a new one, preventing duplicate credits.
</Info>
...
- Check that your `shopifyOrderIdField` configuration is correct
...
**Solution:**
- Check the return metadata in Redo for the existing Brightpearl sales credit ID
- Verify the sales credit in Brightpearl using the credit ID
...
<AccordionGroup>
<Accordion title="Sales Order Lookup Process">
When creating a sales credit, Redo:

1. Takes the Shopify order ID from the return (e.g., `1234567890`)
2. Formats it as `SO1234567890`
3. Searches Brightpearl sales orders using the configured field (e.g., `customerRef`)
4. Retrieves the matching sales order with all line items
5. Extracts customer, channel, and pricing information
</Accordion>

<Accordion title="Product Line Matching">
For each returned product:

1. Redo identifies the product SKU from the return
2. Searches the original sales order lines for a matching SKU
3. Retrieves the product ID, tax code, and pricing from the original line
4. Calculates the return amount based on quantity and original pricing
5. Preserves tax codes and customer-specific pricing
</Accordion>

<Accordion title="Sales Credit Structure">
The sales credit created in Brightpearl includes:

- **Customer ID:** From the original sales order
- **Parent Order ID:** References the original sales order
- **Channel ID:** Same channel as the original order
- **Reference:** Original order reference/name
- **Credit Lines:** One line per returned product with:
  - Product ID and SKU
  - Quantity being returned
  - Net amount (excluding tax)
  - Tax amount
  - Tax code from original order
</Accordion>

<Accordion title="Financial Calculations">
Return totals are calculated using Redo's return totals calculator:

- Product value excludes taxes and adjustments
- Tax amounts are calculated separately
- Values match the original transaction proportionally
