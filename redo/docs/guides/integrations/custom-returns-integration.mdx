...
Include `refunds` on the order sync payload when refunds already happened in
your system. Each refund needs a stable `id` and `createdAt`, plus any of:

- `lineItems` — refunded quantities and amounts for specific lines
- `shippingRefunds` — shipping refunded outside line items
- `orderLevelRefunds` — discretionary or order-wide refunds

Line item refund amounts follow the same tax rules as the rest of the order:
`subtotalSet` is pre-tax and `totalTaxSet` is the refunded tax.

To add or update refunds later without resending the full order, call
`POST /stores/{storeId}/custom/orders/{orderId}/refunds` with one or more
refunds. Matching `id` values replace the existing refund; new `id` values are
appended.

<Warning>
Syncing refunds does not update the order's `current_*` totals (price,
subtotal, tax). Redo uses refund `lineItems` to reduce returnable quantity and
recalculates refund amounts from line item prices at return time — not from
`current_total_price`. If you need post-refund totals reflected elsewhere,
re-sync the full order with updated line item quantities or amounts.
</Warning>
...
| Reason | Use when |
| --- | --- |
| `customer` | The shopper asked to cancel the order. |
| `fraud` | You cancelled the order because you judged it fraudulent. |
| `inventory` | You could not supply the items, so you cancelled the order. |
| `declined` | Payment was declined, so the order was never completed. |
| `staff` | Someone on your team cancelled it, including to correct an order-entry mistake. |
| `other` | Any reason the values above do not describe. |
