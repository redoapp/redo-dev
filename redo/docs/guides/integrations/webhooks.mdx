    ...
    "items": [
      {
        ...
        "upc": "012345678905",
        "processingImages": [
          "https://files.getredo.com/images/inspection-1.jpg"
        ],
        "customerImages": [
          "https://files.getredo.com/images/customer-damage-1.jpg"
        ]
      }
    ],
    "source": {
...
The public return API and the `return` webhook topic expose the following image
arrays on each `return.items[]` item:

<ResponseField name="return.items[].processingImages" type="array">
  Image URLs added by the merchant or warehouse while processing the return item. This does not include customer-uploaded images. The array is empty when no processing images have been added.
</ResponseField>

<ResponseField name="return.items[].customerImages" type="array">
  Image URLs uploaded by the customer for the return item. These come from image-type input answers in the customer's return flow. The array is empty when no customer images have been uploaded.
</ResponseField>
