...
### Analyze Commerce Agent conversations

**Prompt:**

> Summarize the themes, shopper objections, and unanswered questions in our 20 most recent Commerce Agent conversations.

The AI will call `list_commerce_agent_conversations` for the recent conversations, then call `get_commerce_agent_conversation` for the transcripts it needs to support the analysis.

### Investigate Recover conversations

**Prompt:**

> Show me replied Recover conversations triggered by checkout abandonment and explain what shoppers asked after the outreach.

The AI will call `list_commerce_agent_conversations` with `status='replied'`, `author='recover'`, and `triggerId='checkout_abandoned'`, then inspect matching transcripts with `get_commerce_agent_conversation`.
...
The AI calls `generate_custom_analytics_report` with the question and waits for
the result. When the status is `ready`, it shares the returned `reportUrl`.
You can use that URL to view the report or download a CSV. The AI does not need
to poll `get_custom_analytics_report`.

If the tool returns `needs_clarification`, it does not create a report or return
a report URL. The AI explains the limitations and the next action:

- With a `confirmationId`, the AI asks for your approval of the proposal and its limitations.
- Without a `confirmationId`, the AI asks you to revise the question.

After your explicit approval, the AI calls the same tool with the original
question and the returned `confirmationId`. If you change the question, the AI
omits `confirmationId` and requests a new proposal.

The AI also explains that compilation validation does not execute report rows
or guarantee runtime success.
...
The AI calls `edit_custom_analytics_report` with the report ID and a full
replacement spec. The spec includes all desired metrics, dimensions, filters,
and dates, not just the changed fields.

The tool validates the query before changing the report and waits for the
update. A successful response includes status `ready` and a report URL, without
polling. If the request requires unsupported changes or limitations, the report
stays unchanged and the AI asks for a revised spec. The edit tool does not
accept `confirmationId`.
