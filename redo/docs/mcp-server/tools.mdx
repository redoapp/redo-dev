...
| Tool | Description | Required permission |
|---|---|---|
| **generate_custom_analytics_report** | Create a report from a plain-English question and provide a link when it is ready. The AI asks for clarification or approval when needed. | Other |
| **edit_custom_analytics_report** | Update an existing report and wait for completion. If the request cannot be supported, the report stays unchanged. | Other |
| **get_custom_analytics_report** | Get a report's current status, title, requested question, columns, and report link when ready. | Other |
