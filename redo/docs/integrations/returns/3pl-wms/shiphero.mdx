...
- **Processable statuses**: Statuses that tell Redo to process the return. Defaults to **Complete** and **Warehouse Complete**; leave both unselected to turn off automatic processing.
- **Trust RMA status only**: Process all items as soon as ShipHero marks the RMA complete, without checking per-item received quantities. Use this if your warehouse doesn't report received counts.
- **ShipHero statuses that mean rejected**: Free-text statuses that mark the return rejected in Redo. Case-insensitive.
- **ShipHero statuses that flag for manual processing**: Free-text statuses that hold the return for your team to review instead of processing it.
...
- **Restock items in ShipHero through Redo**: When you process a return in Redo, restocked items are added back to ShipHero inventory in the bin chosen for each item.
- **Exchange hold delay (seconds)**: How long Redo waits after creating an exchange order before placing the ShipHero fulfillment hold, giving ShipHero time to import the order first. Defaults to 45 seconds.
- **Common bins**: Saved bin names, offered when you pick where to restock an item. Use **Validate** to confirm a bin exists in ShipHero.
