...
**Automatic via webhook**. ShipHero sends return status changes to Redo as warehouse staff receive and process the return.
</Step>
...
- **ShipHero statuses that mean rejected**: Free-text statuses that mark the return rejected in Redo.
...
Status matching is case-insensitive, and a given status can only appear in one of these three lists.
