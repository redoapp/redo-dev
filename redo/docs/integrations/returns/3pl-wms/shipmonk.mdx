...
Your products and orders must already be synced to ShipMonk, typically through your e-commerce platform (Shopify, BigCommerce, etc.). By default, returns cannot be created for orders that don't exist in ShipMonk's system. If some of your orders are fulfilled outside ShipMonk, see the **Blind returns fallback** setting below.
</Accordion>
</AccordionGroup>
...
- **Blind returns fallback** (optional): Create the RMA in ShipMonk even when the original order can't be found there, for example orders fulfilled by another warehouse or merged by ShipMonk. The RMA is built from the return's SKUs, quantities, and tracking number, but is not linked to a ShipMonk order.
